...
journals.get("/:qty", async (req, res, next) => {
    ...
    try {
        ...
        const populatedJournals = await JournalSchema.populate(randomJournals, {
            path: "trip",
            populate: { path: "user" },
        })
...
journals.get("/id/:journalID", verifyToken, async (req, res, next) => {
    ...
    try {
        const journal = await JournalSchema.findById(journalID).populate({
            path: "trip",
        })
...
journals.get("/slug/:journalSlug", async (req, res, next) => {
    ...
    try {
        const journal = await JournalSchema.find({
            slug: journalSlug,
        }).populate({ path: "trip" })
        res.status(200).json(journal)
    } catch (e) {
