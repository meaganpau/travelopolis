...
users.post('/update', async (req, res, next) => {
    ...
    try {
        ...
        if (match) {
            User.findOneAndUpdate( { _id: _id }, { password, firstName, lastName, slug }, (err, user) => {
                if (err) {
                    if (err.name === 'MongoError' && err.code === 11000) {
                        // Slug in use
                        return res.status(500).send({ success: false, field: 'slug', message: 'Oops! That slug is already in use. 🧐'})
                    }
                    return res.status(500).send(err)
                } else {
                    res.status(200).json({ 
            ...
            return res.status(500).send({ success: false, field: 'old_password', message: 'Incorrect password 🙅🏻‍♀️'})            
        }
      } catch (e) {
        next(e)
      }
