    ...
    getJournalData = async (tripID) => {
        try {
            ...
            if (res.data.length) {
                ...
                this.setState({
                    status: `Looks like ${this.state.user.firstName} hasn't written any journals yet 🙁. Check back later!`,
                })
            }
        } catch (e) {
            console.log(e)
        ...
        return (
            <React.Fragment>
                ...
                <ContentContainer>
                    ...
                    <InnerContainer>
                        {trip ? <DoubleTitle>{trip.name}</DoubleTitle> : null}
                        {journals.length ? (
                            <JournalList>
                                {journals.map((journal) => {
                                    return (
                                        <li key={journal.slug}>
                                            <JournalLink
                                                to={`/${userSlug}/${trip.slug}/${journal.slug}`}
                                            >
                                                <Card
                                                    name={journal.title}
                                                    subtext={formatDate(
